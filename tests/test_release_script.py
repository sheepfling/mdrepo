...
def test_release_build_runs_build_and_twine_checks(
    ...
    version = release.project_version()
    stale = tmp_path / f"{release.PACKAGE_NAME}-0.1.0-py3-none-any.whl"
    ...
    def fake_run(command: list[str], **_kwargs: object) -> subprocess.CompletedProcess[str]:
        ...
        if "build" in command:
            (tmp_path / f"{release.PACKAGE_NAME}-{version}.tar.gz").write_bytes(b"sdist")
            (tmp_path / f"{release.PACKAGE_NAME}-{version}-py3-none-any.whl").write_bytes(b"wheel")
